  ...
  // Prepare List of Custom Variables for MC or MM
      ...
      // Make the Email
          ...
          while ($i<$nos)
          {
            ...
            {
              $email_body .= "<tr><td style='$label_style'> ".$new[$i]['label']."</td><td style='$value_style'>".htmlentities($new[$i]['value'], ENT_QUOTES, "UTF-8")."</td></tr>";
            }
          ...
          if ($con[0]['mail_type']=='smtp')
          {
            ...
            if ($autoreply)
            {
              foreach ($autoreply as $ar_to)
              {
                if (!(empty($ar_to)))
                {
                  $con['0']['autoreply_s'] = '=?utf-8?B?'.base64_encode($con['0']['autoreply_s']).'?=';
