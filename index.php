...
if (!($query)) {
    // Create Table(s)
    $query = ORM::for_table('add_table')->create();
    $query->application = 'mailchimp';
    $query->save();
    $query = ORM::for_table('add_table')->create();
    $query->application = 'campaign';
    $query->save();
    $query = ORM::for_table('add_table')->create();
    $query->application = 'aweber';
    $query->save();
}
    ...
    <form class="modal hide fade" id='new_form' action='javascript:submit_new_form();' style='width: 640px'>
        ...
        <div class="modal-body">
            ...
            <label class='label_radio circle-ticked'>
                <input type='radio' value='new' checked name='type_form'>
                <div class='label_div' style='background: #fff'>New Form</div>
            </label>
            <label class='label_radio circle-ticked' style='margin-left: 25px'>
                <input type='radio' value='duplicate' name='type_form' id='rand_aa'>
                <div class='label_div' style='background: #fff'>Duplicate</div>
            </label>
            ...
            <label class='label_radio circle-ticked' style='margin-left: 40px'>
                <input type='radio' value='import' name='type_form' id='rand_b'>
                <div class='label_div' style='background: #fff'>Import</div>
            </label>


            <span class='import btn btn-success fileupload-cover'>
						<input id="fileupload" class='import import_field fileupload' type="file" name="files[]"
                               data-url="file-upload/server/php/" multiple
                               style='width: 84px; height: 24px; margin: 0px;'>
						<span style='position: absolute; left: 0px; right: 0px; text-align: center; top: 0px; font-size: 12px; line-height: 30px'
                              id='import_field_label'>
							Upload Form
						</span>
					</span>
            ...
            <input id='form_name_1' name='name' type="text" class="input-small" autofocus placeholder='Site Feedback'
                   style='width: 220px'>
        ...
        <div class="modal-footer">
            ...
            <button type="submit" id='submit_new_btn' class="btn btn-success"><i class='icon-plus icon-white'></i> Add
                Form
            </button>
        </div>
    </form>
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane" id="forms">
            ...
            <div class='group_cover'>
                ...
                <a class='btn btn-success' id='new_form_pop' data-toggle='modal' href='#new_form'
                   style='margin-left: 10px; margin-bottom: 10px; font-weight: bold; font-size: 15px; padding: 10px 20px'><i
                            class='icon-plus icon-white'></i> Add Form</a>
                        ...
                        <table style='table-layout: fixed' class='table table-hover table-striped' id='ext'>
                            ...
                            foreach ($myrows as $row) {
                                ...
                                <tr id='<?php echo $row['id']; ?>'>
                                    ...
                                    <td class='row_click'><a class='rand'
                                                             href='builder.php?id=<?php echo $row['id']; ?>'><?php echo $row['name']; ?></a><input
                                                class="rand2" style="width: 110px; display:none; margin-right: 6px"
                                                type="text" value="<?php echo $row['name']; ?>"><a class='btn edit_btn'
                                                                                                   title='Edit Form Name'
                                                                                                   id='edit_<?php echo $row['id']; ?>'>edit</a><a
                                                class='btn save_btn' id='edit_<?php echo $row['id']; ?>' type='submit'>save</a>
                                    </td>

                                    <td class='row_click row_description'><a
                                                class='rand'><?php echo $row['description']; ?></a></td>

                                    <td style='text-align: center; border-right: 1px solid #eee'>[formcraft
                                        id='<?php echo $row['id']; ?>']
                                    </td>
                                    ...
                                    <td style='text-align: center'>
                                        ...
                                        <a href='php/exportOne.php?id=<?php echo $row['id']; ?>' target='_blank'
                                           title='Export all submissions for this form' class='export_alt'><i
                                                    class='icon-share-alt icon-2x'></i> </a>
                                        <a class='delete-row' data-loading-text='...'
                                           data-complete-text="<i class='icon-ok icon-2x'></i>"
                                           id='delete_<?php echo $row['id']; ?>' title='Delete this form'><i
                                                    class='icon-trash icon-2x'></i>
                                        </a>
                                    </td>
                                </tr>
                            <?php } ?>
        ...
        <div class="tab-pane" id="submissions">
            <div class='group_cover'>
                <div style='border-bottom: 1px solid #eee; padding-bottom: 15px; margin-bottom: 10px'>
									<span class='stat'>
										<span class='unr_msg'
                                              id='unr_ind'><?php echo sizeof($mysub) - sizeof($mysubr); ?>
										</span> unread&nbsp;&nbsp;
                    ...
                    <a class='btn btn-success' id='export' style='margin-left: 30px'
                       title='Export all submissions data to CSV' target='_blank' href='php/exportAll.php'>
                        Export Data to CSV
                    </a>
                </div>
                        ...
                        <table style='table-layout: fixed' class='table-sub table table-hover' id='files_manager_table'>
                            ...
                            foreach ($read['files'] as $key => $value) {
                                ...
                                <tr>
                                    ...
                                    <td><a class='delete_from_manager' style='width: 38px' data-loading-text='...'
                                           data-url='<?php echo $value['url']; ?>'
                                           data-complete-text='<i class="icon-ok icon-2x"></i>'
                                           id='del_fm_<?php echo $key + 1 ?>'><i class='icon-trash icon-2x'></i></a>
                                    </td>
                                </tr>
                            <?php } ?>
        ...
        <div class="tab-pane" id="add">
            ...
            if (defined('FORMCRAFT_ADD')) {
                ...
                <div style='width: 100%; margin: auto auto; text-align: center; font-size: 24px; color: #666; font-weight: 300; line-height: 132%'>
                    Install <a href='http://wordpress.org/plugins/formcraft-add-on-pack/' target='_blank'>FormCraft
                        Add-On Pack</a><br>
...
//        print_r($value[value]);
    ...
    <span style='display: none' id='upd_name_<?php echo $row['id']; ?>'><?php if (!(empty($name))) {
            echo $name;
        } else {
            echo '(form deleted)';
        } ?></span>
...
<div class='hid modal hide fade' id='view_modal' aria-hidden="true">
    ...
    <div class="modal-footer">
        <button value="Print Div" class='btn btn-primary' onclick="PrintElem('#print_area')"/>
        Print
        </button>
